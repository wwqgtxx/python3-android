...
./configure CROSS_COMPILE_TARGET=yes HOSTPYTHON="$(pwd)/hostpython" CONFIG_SITE=config.site --prefix="${PREFIX}" --host="${TARGET}" --build="${HOST}" --disable-ipv6 --without-ensurepip || exit 1

# pause before actually make
echo " ---> pause BEFORE start make "
bash
